...
module Prims : PRIMS = struct
  ...
  set_cdr:
    ...
    mov rdx, PVAR(0)
    mov rcx, PVAR(1)
    mov [ rdx + WORD_SIZE + TYPE_SIZE ], rcx
    mov rax, SOB_VOID_ADDRESS
  ...
  apply:
    ...
    mov rdx, 0            ; args counter
    mov rbx, [ rbp + WORD_SIZE * 3 ]         ; args number
    mov r10, [ rbp + WORD_SIZE * (rbx + 3) ]   ; list pointer
    mov r11, [ rbp + WORD_SIZE * (rbx + 3) ]   ; list pointer                               ; list pointer
    loop:
      cmp r10, SOB_NIL_ADDRESS     ; check if list nil
      je reverse_array
      CAR rbx, r10              ; first of list
      CDR r10, r10              ; rest of list
      push rbx
      add rdx, 1
      jmp loop

    reverse_array:
      mov r12, rdx       ; r11 = address of array, r9 n r12 = args counter
      mov r9, rdx     ; total args counter?
      dec r9
      lea r8, [ r11 + WORD_SIZE * r9 ]      ; address of last entry
      cmp r11, r8                ; check if small array (0/1)
      jnb .done                 
      .next:

        mov rax, [r11]              ;rax = value at start
        mov rbx, [r8]              ;rbx = value at end
        mov [r8] ,rax              
        mov [r11] ,rbx              
        add r11, WORD_SIZE                  ;r11 = address of next item at start
        sub r8, WORD_SIZE                  ;r8 = address of next item at end
        cmp r11, r8                ; if in the middle
        jb .next                   
      .done:
  ...
  let procs = String.concat "\n\n" [type_queries ; numeric_ops; misc_ops; funcs];;
end;;
